...
Phase 1a - 1b subagents must all complete before going to phase 1c.

### 1c. Tunnel

Run the `start-tunnel` skill in a **background** subagent. Pass it the **port** from step 1a.
It returns the **tunnel ID** and **frpc PID**.

## Phase 2: Project Creation

Call the `create_project` tool with the problem statement and tunnel ID from Phase 1. Share the `project_url` with the user (e.g., `[View in Softlight →](<project_url>)`) and remember the `project_id` for future interactions.
...
each with a `prompt_id`, `slot_id`, and full `text`. Only prompts that have **not** already been
completed are returned, so on second+ revisions older prompts are automatically excluded.
...
2. Instructions to mark the prompt as done when finished by calling `complete_prompt` with
