...
### Phase 2: Upload content scripts and collect attachments

For each subagent that returns a file path, use the `create-attachment` skill to upload the
content script file to Drive. This returns an attachment object `{name, url}`. Collect all
attachment objects — these are what you pass to the MCP tool.
...
item. The `content_script` field is now an **attachment object** (not a string):

```
generate_prototype_revision(
  project_id="<project_id>",
  elements=[
    {"url": "<tunnel_url>", "title": "<title>", "content_script": {"name": "content_script.js", "url": "<drive_url>"}},
    {"url": "<tunnel_url>", "title": "<title>", "content_script": {"name": "content_script.js", "url": "<drive_url>"}},
    ...
  ]
)
...
different `content_script` attachment that implements its design direction. The canvas renders
each iframe independently, so the user sees multiple variations of the same app side by side.
