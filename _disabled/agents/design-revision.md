...
description: "Respond to PM feedback on the canvas — read comments, create targeted explorations, and present the next iteration."
...
The PM has reviewed your design exploration on the canvas and left feedback. Your job is to
read their comments, understand what they're asking for, and do the deep thinking a senior
product designer would do to create the next round of explorations that directly responds to
their feedback.
...
Pass to `complete_prompt` when all work is done.

### `<prompt_text>`

What the user requested. This may be generic — the real feedback is in the canvas comments.
...
You create explorations and kick off content scripts. The `present-revision` agent handles
everything the human reads — narrative text, spatial organization, and framing the new work
as a response to the PM's feedback.

**Use `present-revision`, NOT `present-canvas`.** `present-canvas` is for the initial
exploration and will reposition all slots on the canvas, breaking existing layout.
`present-revision` only positions new slots and writes narrative that connects PM feedback
to the new work.

**Dispatch `present-revision` FIRST — before content scripts.** The presenter is a small,
...
2. Dispatch `present-revision` in the background with your analysis and what you created
...
Dispatch the `present-revision` agent in the background:
...
## Your task: respond to feedback with design work
...
1. **Understand where things stand.** Call `get_project`. Check for completed prototypes,
   ...
   thread to understand where the discussion landed.

2. **Decide what to do next.** Based on the PM's feedback, figure out what to work on next. Your explorations should directly respond to their feedback.

3. **Do the work.** Create explorations, then dispatch `present-revision` first (it's fast and
   keeps the canvas alive), then fan out content-script subagents. When you finish, the canvas
   should show clear progress on what the PM asked for.

4. **When all work is done, call `complete_prompt`** with the `project_id` and `prompt_id` to
   dismiss the loading state on the canvas.
