...
IMPORTANT: For anything that is new in the prototype that has affordance, i.e it is clickable, we need to build the expected behavior as much as possible. For anything that is intended to open up something existing in the UI add a tooltip on click describing where that CTA connects to in the existing codebase. For actions surfaced to the user to take that is not mocked, explain why it's pending in a similar click tooltip; or go back and reassess if we really need it in the first place given rule 5. Use a shake on the clickable element to denote that this is an interaction that is not yet mocked but expected to be mocked in the future. Make the tooltip an alert/accent color following the softlight design system to make it clear that this is not a real interaction but an expected one.
